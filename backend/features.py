...
NUMERIC_FEATURES_FULL = NUMERIC_FEATURES_NO_SPATIAL + SPATIAL_FEATURES
...
def enrich_frame(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for feature in ("factories_2km", "factories_5km", "factories_10km"):
    ...
    enriched["factory_density_2km"] = enriched["factories_2km"].astype(float) / (np.pi * 2.0**2)
    enriched["factory_density_5km"] = enriched["factories_5km"].astype(float) / (np.pi * 5.0**2)
    enriched["factory_density_10km"] = enriched["factories_10km"].astype(float) / (np.pi * 10.0**2)
...
def enrich_row(row: dict[str, Any], when: pd.Timestamp | None = None) -> dict[str, Any]:
    ...
    for radius in (2, 5, 10):
        count_key = f"factories_{radius}km"
        count = float(enriched.get(count_key, 0.0) or 0.0)
        enriched[count_key] = count
        enriched[f"factory_density_{radius}km"] = count / (np.pi * radius**2)
